...
static char kuipos[8][2] = {
        {0, 0}, {1, 0}, {2, 0}, {3, 0}, {0, 1}, {0, 2}, {2, 2}, {0, 3},
};
...
static int ChainTable2[][2] = {
        ...
        {-4, -7}, {-8, -4}, {-8, -3}, {-8, -2}, {-8, -1}, {-8, 0},
        {-8, 1},  {-8, 2},  {-8, 3},  {-8, 4},  {0xFFFF},
};

//ε
static int ChainTable3[][2] = {
        {6, 6}, {-6, 6}, {6, -6}, {-6, -6}, {0xFFFF},
};

static void GetChainTable(int x, int y, int (**PPout)[2], bool* isYmirror,
                          bool* isXYswap) {
    RotateCorrect(&x, 4);
    RotateCorrect(&y, 4);
    if (x == 0 && y == 0) {  //α
...
//					ia = ia ;
                ...
                if (il != -1) {
                    BYTE* Ptmp;
                    Ptmp = &working.exptile[editingcn][(ix + iy * FZCD_CHIP_P_MAP_W) *
                                                       FZCD_SOE_CHIP];
                    int wv;
                    wv = ia - 0xCC380;
                    Ptmp[0] = wv;
...
bool TF_mm_kui(TCB* caller) {
    ...
    switch (mmkuistate) {
        case 0: {
            ...
            if (gmx >= 0 && gmx < FZCD_TILE_P_MAP_W && gmy >= 0 &&
                gmy < FZCD_TILE_P_MAP_H) {
                int i = 0;
                for (; i < 8; i++) {
                    ...
                    for (int ia = ia0 + 1; ia < ia0 + noloop; ia++) {
                        ...
                        for (; it < 4; it++) {
                            if (memcmp(kuidata[cmptile[it]],
                                       kuidata[GETROM8(ex + it + 0xCC380)], 4))
                                break;
                        }
                        if (it == 4) {
                            if (KeyOn(KC_CTRL)) {
...
#define MOVE_KUI(xv, yv)                                          \
    ;                                                             \
    if ((xv) >= 0 && (xv) < FZCD_TILE_P_MAP_W * 4 && (yv) >= 0 && \
        (yv) < FZCD_TILE_P_MAP_W * 4) {                           \
...
#undef KUI_MOVE
        ...
        case 1: {
            ...
            TM.KillP(mmkuiPkoho, TP_HIGHEST, TP_LOWEST);
            {
                ...
                for (int i = 0; Pct[i][0] != 0xFFFF; i++) {
                    TCB* Ptcb;
                    Ptcb = TM.AddTask(mmkuiPkoho, TP_DUMMY, NULL, NULL,
                                      TV_mm_kui_token);
                    if (!Ptcb) return true;
            ...
            if (MousePush(MB_L)) {
                ...
                for (int i = 0; i < 8; i++) {
                    if ((tx % 4) == kuipos[i][0] && (ty % 4) == kuipos[i][1]) {
                        //					mmkuiPkui[
                        //(ty/4)*(FZCD_TILE_P_MAP_W+1)+(tx/4) ] ^= (1<<i) ;
                        memmove(mmkuiPkuilog + 2, mmkuiPkuilog + 0,
                                sizeof(__int32) * (LOG_TIMES - 1) * 2);
    ...
    if (mmkuipenX != mmkuipenXb || mmkuipenY != mmkuipenYb) {
