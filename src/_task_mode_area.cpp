    ...
    if (dragging) {
        ...
        switch (dragging) {
            case 1: {
                ...
                }
                {
                    ...
                    if (tmp) {
                        ...
                        Pdest->y = Pdest->y / 8 * 8;
                        //						else
                        //							working.area[editingcn][selected].y
                        //= working.area[editingcn][selected].y/0x100*0x100+0x80 ;
                        RequestRedraw();
                    }
                }
            } break;
            case 2: {
                ...
                {
                    double angle;
                    angle = atan2(my - Pdest->y, mx - Pdest->x);
                    angle += (PI / 8);
                    AngleFixPlus(&angle);
                    int tmp;
                    tmp = (int)(angle / (PI / 4));
        ...
        if (MousePush(MB_L) && KeyOn(KC_CTRL)) {
            ...
            if (isstartline) {
                Pdest->x = (Pdest->x) / 0x100 * 0x100 + 0x80;
                //				working.area[editingcn][i].y =
                //(working.area[editingcn][i].y)/0x100*0x100+0x80 ;
            ...
            if (!isstartline) {
                double angle;
                angle = atan2(Pdest->y - (Pdest - 1)->y, Pdest->x - (Pdest - 1)->x);
                angle += (PI / 8);
                AngleFixPlus(&angle);
                    ...
                    for( int i=0 ; i<FZCD_MAX_AREA ; i++ )
                    {
                            if( working.area[editingcn][i].isexist )
                            {
                                    if( working.area[editingcn][i].x % 8 )
                                            MessageBox( hWnd , Int2CharEX( i ) , "hosei"
       , MB_OK ) ; if( working.area[editingcn][i].y % 8 ) MessageBox( hWnd , Int2CharEX(
       i ) , "hosei" , MB_OK ) ; working.area[editingcn][i].x =
       working.area[editingcn][i].x/8*8 ; working.area[editingcn][i].y =
       working.area[editingcn][i].y/8*8 ;
                            }
                    }
...
bool ModeArea ::SelectArea(int scx, int scy) {
    ...
    for (int i = 0; i < FZCD_MAX_AREA; i++) {
        if (working.area[editingcn][i].isexist) {
            if (MYABS(MYABS(working.area[editingcn][i].x - mx) +
                      MYABS(working.area[editingcn][i].y - my)) < 20) {
                selected = i;
                return true;
            }
        }
        if (working.area_sub[editingcn][i].isexist) {
            if (MYABS(MYABS(working.area_sub[editingcn][i].x - mx) +
                      MYABS(working.area_sub[editingcn][i].y - my)) < 20) {
                selected = i + 0x1000;
                return true;
            }
        }
            ...
            if (i != 0 || sub) {
                {
                    ...
                    if (sub) {
                        AREA* Pdestm = working.AreaGetSelectedArea(
                                editingcn, i + working.area_sub_org + 1);
                        if (Pdestm->isexist) {
                            int mtx, mty;
                            mtx = (int)(Pdestm->x * TRUE_MULTI) + scx * MAP_TILE_SIZE;
                            mty = (int)(Pdestm->y * TRUE_MULTI) + scy * MAP_TILE_SIZE;
                            mdo.Line(MDO3normal, SFC_BACK, mtx, mty, tx, ty,
                                     myRGB(0, 0, 31));
                        }
                    }
                ...
                int tty;
                tty = (int)((Pdest->y / 0x100 * 0x100) * TRUE_MULTI) +
                      scy * MAP_TILE_SIZE;
                mdo.Cls(MDO3normal, SFC_BACK, tx - 1, tty, 3, (int)(0x200 * TRUE_MULTI),
                        myRGB(31, 0, 0));
...
static void TV_me_put(TCB* caller) {
...

    {
        if (show_area_scope && selected > 0) {
            ...
            //		KAGEMOJI( SFC_BACK , "Tab：エリア範囲表示切り替え" , 0 ,
            // WINHEIGHT-14*2 , 14 , 7 ) ;
        }
    }
