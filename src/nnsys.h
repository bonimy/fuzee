...
inline __int32 FMulti(int db, __int32 a, __int32 b) {  //若干aのほうが精度が高いことが
...
// atan2を知らなかった頃に作った奴
extern double AimAngle(double, double, double, double);  //２点間の角度
...
extern void smrnd(int seed,
                  unsigned char* Pes =
                          NULL);  //適当なシードと『外乱』へのポインタ（NULL可）を入れる
extern unsigned int mrnd(MYRND_PARAM* Pmrp);  // 0～0xFFFFFFFFまでの整数を返す
extern double dmrnd(MYRND_PARAM* Pmrp);       //[0,1)の小数を返す
extern void smrnd(MYRND_PARAM* Pmrp, int seed,
                  unsigned char* Pes =
                          NULL);  //適当なシードと『外乱』へのポインタ（NULL可）を入れる
extern unsigned int EHash(unsigned char* Pd,
                          int length);  //かなりテキトーな２４ビットハッシュ（？）値作成
