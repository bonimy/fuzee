...
inline void MyDIBObj3 ::Line_sub2(bool isXY, double sloped, const MDO3Opt* opt,
    ...
    if (pdb < cb) {        //始まりの位置で、ｂ負方向にはみ出ている
        if (expeb < cb) {  //終わりの位置で、ｂ負方向にはみ出ている
            return;
        } else if (expeb >= cbs) {  //終わりの位置で、ｂ正方向にはみ出ている
            if (!EitherCorrect(&sa, &ea, &pdb, cb, cbs, pdb, slope)) return;
        } else {  //終わりの位置ははみ出していない
            int tmp;
            tmp = DivideCorrectR(sa, ea, cb, cbs, pdb, slope);
            pdb += ((tmp - sa) * slope);
            sa = tmp;
        }
    } else if (pdb >= cbs) {  //始まりの位置で、ｂ正方向にはみ出ている
        if (expeb < cb) {  //終わりの位置で、ｂ負方向にはみ出ている
            if (!EitherCorrect(&sa, &ea, &pdb, cb, cbs, pdb, slope)) return;
        } else if (expeb >= cbs) {  //終わりの位置で、ｂ正方向にはみ出ている
            return;
