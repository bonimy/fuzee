...
char *Ptmp ;
	...
	if( mmcurrentbandsize == -1 )
		sprintf( &buf[strlen(buf)] , "   Band \?\?\?\?/%X  CHIP \?\?\?\?/%X" ,  FZCD_BAND_SPACE_LIMIT , FZCD_CHIP_SPACE_LIMIT ) ;
	else
		sprintf( &buf[strlen(buf)] , "   Band %X/%X  CHIP %X/%X" , mmcurrentbandsize , FZCD_BAND_SPACE_LIMIT , mmcurrentchipsize , FZCD_CHIP_SPACE_LIMIT ) ;
...
bool TF_mm_set(TCB *caller)
{
	...
	if( dragging )
	{
		...
		case 10:
			...
			if( gmx<0 || gmy<0 || gmx>FZCD_CHIP_P_MAP_W || gmy>FZCD_CHIP_P_MAP_H )return false ;
			{
				int tmp ;
				tmp = (gmx-clipx)-clipw ;
				if( tmp )
...

	{
		if( KeyPush( KC_DEL ) )
		{
			if( clipw<0 ){clipx+=clipw;clipw*=-1;}
			if( cliph<0 ){clipy+=cliph;cliph*=-1;}
			BYTE src[2] ;
			src[0] = selected ;
			src[1] = selected>>8 ;
			...
			{
BYTE *Ptmp ;
				Ptmp = &working.exptile[editingcn][(clipx+ix+(clipy+iy)*FZCD_CHIP_P_MAP_W)*2] ;
	...
	}
	{
		if( KeyPush( KC_C ) || KeyPush( KC_INS ) )
		{
			...
			{
BYTE *Ptmp ;
		Ptmp = &working.exptile[editingcn][(clipx+(clipy+iy)*FZCD_CHIP_P_MAP_W)*2] ;
	...
	if( Pclip )
	{
		...
		if( MouseOn( MB_L ) )
		{
			...
			for( int ix=0 ; ix<clipw ; ix++ )
			{
				if( gmx+ix>=FZCD_CHIP_P_MAP_W ||
					gmy+iy>=FZCD_CHIP_P_MAP_H )continue ;
BYTE *Ptmp,*Psrc ;
				Psrc = Pclip+(ix+iy*clipw)*2 ;
				if( Psrc[0]==0xFF && Psrc[1]==0xFF )continue ;
				Ptmp = &working.exptile[editingcn][(gmx+ix+(gmy+iy)*FZCD_CHIP_P_MAP_W)*2] ;
...
BYTE *Pbuf ;
			...
			if( counter%2 == 1 )
			{
				for( int iy=0 ; iy<cliph ; iy++ )
				{
					for( int ix=0 ; ix<clipw ; ix++ )
					{
						Pbuf[(iy*clipw+ix)*2  ] = Pclip[(iy*clipw+clipw-1-ix)*2  ] ;
						Pbuf[(iy*clipw+ix)*2+1] = Pclip[(iy*clipw+clipw-1-ix)*2+1] ;
					}
				}
			}
			else
			{
				for( int iy=0 ; iy<cliph ; iy++ )
				{
					for( int ix=0 ; ix<clipw ; ix++ )
					{
						Pbuf[(iy*clipw+ix)*2  ] = Pclip[((cliph-1-iy)*clipw+ix)*2  ] ;
						Pbuf[(iy*clipw+ix)*2+1] = Pclip[((cliph-1-iy)*clipw+ix)*2+1] ;
					}
				}
			}
...
BYTE *Pbuf ;
			...
			for( int iy=0 ; iy<clipw ; iy++ )
			{
				...
				{
					Pbuf[(iy*cliph+ix)*2  ] = Pclip[((cliph-ix-1)*clipw+iy)*2  ] ;
					Pbuf[(iy*cliph+ix)*2+1] = Pclip[((cliph-ix-1)*clipw+iy)*2+1] ;
				}
			}
			memcpy( Pclip , Pbuf , clipw*cliph*2 ) ;
			free( Pbuf ) ;
		...
		case 1:
			{
				...
				if( memcmp( dest , src , 2 ) )
				{
					...
					{
						BYTE *Ptmp ;
						Ptmp = &working.exptile[editingcn][(gmx+iy*FZCD_CHIP_P_MAP_W)*2] ;
		...
		case 2:
			{
				...
				if( memcmp( dest , src , 2 ) )
				{
					...
					{
						BYTE *Ptmp ;
						Ptmp = &working.exptile[editingcn][(ix+gmy*FZCD_CHIP_P_MAP_W)*2] ;
						if( memcmp( Ptmp , dest , 2 ) )break ;
						memcpy( Ptmp , src , 2 ) ;
					}
					...
					for( int timesout=0 ; timesout<100000 ; timesout++ )
					{
						...
						for( int ix=0 ; ix<FZCD_CHIP_P_MAP_W ; ix++ )
						{
							if( Pflag[iy*FZCD_CHIP_P_MAP_W+ix] )
							{
								...
								updated = true ;
								memcpy( &working.exptile[editingcn][(ix+iy*FZCD_CHIP_P_MAP_W)*2] , src , 2 ) ;
								if( ix!=0 &&
									!memcmp( &working.exptile[editingcn][((ix-1)+(iy)*FZCD_CHIP_P_MAP_W)*2] , dest , 2 ) )
										Pflag[(iy+0)*FZCD_CHIP_P_MAP_W+(ix-1)] = 1 ;
								if( ix!=FZCD_CHIP_P_MAP_W-1 &&
									!memcmp( &working.exptile[editingcn][((ix+1)+(iy)*FZCD_CHIP_P_MAP_W)*2] , dest , 2 ) )
										Pflag[(iy+0)*FZCD_CHIP_P_MAP_W+(ix+1)] = 1 ;
								if( iy!=0 &&
									!memcmp( &working.exptile[editingcn][((ix)+(iy-1)*FZCD_CHIP_P_MAP_W)*2] , dest , 2 ) )
...
int chr ;
			...
			mdo.SBlt( &topt , SFC_BACK , tx , ty , MAP_TILE_SIZE , MAP_TILE_SIZE , SFC_BG , 
				chr%16*8 , chr/16*8 , MAP_TILE_SIZE_D , MAP_TILE_SIZE_D ) ;
		}
		}
	}
...
#define	FS	12
		mdo.Text( 0 , buf , WINWIDTH-SAMPLE_SIZE , WINHEIGHT-SAMPLE_SIZE-FS , FS , FS/2 , RGB(0,0,0) ) ;
		mdo.Text( 0 , buf , WINWIDTH-SAMPLE_SIZE-1 , WINHEIGHT-SAMPLE_SIZE-FS-1 , FS , FS/2 , RGB(255,255,255) ) ;
#undef FS
	}
#undef SAMPLE_SIZE
	ALIAS_END() ;
