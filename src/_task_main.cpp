...
#define SAVE(er)		{working.Save(er);RenderMINIMAP();}
...
void RenderMINIMAP()
{
	...
	for( int ic=0 ; ic<2+(editingre==7) ; ic++ )
	{
		...
		memcpy( tmap , working.exptile[0] , FZCD_CHIP_P_MAP_W*FZCD_CHIP_P_MAP_H*FZCD_SOE_CHIP) ;
		...
		{//繋ぎ換え有り
			for( int i=0 ; i<FZCD_CHIP_P_MAP_H*FZCD_CHIP_P_MAP_W ; i++ )
			{
				...
				{
		memcpy( tmap+i*FZCD_SOE_CHIP , working.exptile[0]+i*FZCD_SOE_CHIP , FZCD_SOE_CHIP) ;
				}
				else
				{
		...
		}
		{
			for( int timesout=0 ; timesout<10000 ; timesout++ )
			{
				...
				for( int iy=ydef ; timesout%2 ? iy<FZCD_CHIP_P_MAP_H : iy>=0 ; iy+=inc )
				{
					for( int ix=xdef ; timesout%2 ? ix<FZCD_CHIP_P_MAP_W : ix>=0 ; ix+=inc )
					{
						int offset ;
						BYTE *Ptmp ;
						int tmp ;
...
#undef LM
		...
		for( int iy=0 ; iy<64 ; iy++ )
		{
			for( int ix=0 ; ix<32 ; ix++ )
			{
				...
				for( int q=0 ; q<64 ; q++ )
				{
					if( flag[ (((64-1-iy)*8+q/8) + ( ix*8 + q%8 ) * FZCD_CHIP_P_MAP_W) ] )
					{
						counter++ ;
					}
				}
...
#define LM(dx,dy)		{if( Pbmp[((iy+dy)<<shift)|(ix+dx)] == myRGB(31,31,31) )	\
							{Pbmp[((iy)<<shift)|(ix)] = myRGB(0,0,0);continue ;}}	/**/
					bool LE,RE,TE,BE ;
					LE = RE = TE = BE = false ;
					if( ix==0 )LE = true ;
					if( iy==0 )TE = true ;
...
double pro;
	...
	((int)(sinline((double)(no%8)/8+0.6666)*14*pro+29*(1-pro))) 
	);
}
...
double pro;
	...
	return RGB(
	((int)(sinline((double)(no%8)/8       )*196*pro+220*(1-pro))) ,
	((int)(sinline((double)(no%8)/8+0.3333)*196*pro+220*(1-pro))) ,
	((int)(sinline((double)(no%8)/8+0.6666)*196*pro+220*(1-pro))) 
	);
}
...
//			ty = mainy+(ibl/32)*16 ;
			...
			if( tx<0 || ty<0 || tx>=WINWIDTH || ty>=WINHEIGHT )continue ;
int data ;

BYTE *Pr = &working.exptile[0][(px/2+py/2*FZCD_CHIP_P_MAP_W)*FZCD_SOE_CHIP] ;
				data = Pr[0]|(Pr[1]<<8) ;
				data = GETROM8(0xCC380+data+q) ;

				mdo.SBlt( MDO3normal , SFC_BACK , tx , ty , MAP_TILE_SIZE , MAP_TILE_SIZE , SFC_BG , 
					data%16*8 , data/16*8+(editingcn!=0)*128 , MAP_TILE_SIZE_D , MAP_TILE_SIZE_D ) ;
			}
			}
		}
	}
			...
			for( int iba=0 ; iba<16 ; iba++ )
			{
				for( int ich=0 ; ich<16 ; ich++ )
				{
					...
					for( int i=0 ; i<FZCD_SOE_CHIP ; i++ )
						if( working.exptile[editingcn][(px+py*FZCD_CHIP_P_MAP_W)*FZCD_SOE_CHIP+i] )
							iba = -1 ;
...
int tx,ty;
			...
			if( tx<0 || ty<0 || tx>=WINWIDTH || ty>=WINHEIGHT )continue ;
int data ;
BYTE *Pr = &working.exptile[editingcn][(px/2+py/2*FZCD_CHIP_P_MAP_W)*FZCD_SOE_CHIP] ;
				data = Pr[0]|(Pr[1]<<8) ;
			...
			mdo.Cls( MDO3normal , SFC_BACK , tx , ty , 3 , 3 , myRGB(31,31,0) ) ;
			{
...

				{
					for( int i=0 ; i<3 ; i++ )
					{
						...
						switch( i )
						{
						case 0:tmp = GETROM8( adr1 ) ;break ;
						case 1:tmp = GETROM8( adr2 ) ;break ;
						case 2:tmp = GETROM8( adr3 ) ;break ;
						}
						...
						mdo.Cls( MDO3normal , SFC_BACK , (int)x0-1 , (int)y0-1 , 3 , 3 , myRGB(31,31,0) ) ;
					}
				}
			}
			if( bx!=-1 && by!=-1 )
...
//*
			...
			mdo.Cls( MDO3normal , SFC_BACK , tx , ty , 6*strlen(buf) , 12 , myRGB(31,31,31) ) ;
...
/*
			...
			mdo.Cls( MDO3normal , SFC_BACK , tx , ty+12 , 6*strlen(buf) , 12 , myRGB(31,31,31) ) ;
			mdo.Text( SFC_BACK , buf , tx , ty+12 , 12 , 6 ) ;
			}
			{
			char buf[16] ;
			sprintf( buf , "%02X" , (GETROM8( adr3 )>>4)&3 ) ;
			mdo.Cls( MDO3normal , SFC_BACK , tx , ty+24 , 6*strlen(buf) , 12 , myRGB(31,31,31) ) ;
			mdo.Text( SFC_BACK , buf , tx , ty+24 , 12 , 6 ) ;
			}
//*/
			adr0++ ;
			adr1++ ;
...
int prlist[7] ;
		{
			...
			for(;;)
			{
				...
				for(;;)
				{
#define ERROR_CONT		{fprintf(flog,"不自然な記述\n「%s」\nがありますが無視して続行します。\n" , buf) ;continue ;}
					char buf[512+10] ;
					char buf2[512+10] ;
					int  rpos ;
					rpos = 0 ;
...
int courseno ;
					...
					else if( !strcmp( buf2 , "MM" ) )
					{
						int gpno,cono,dx,dy ;
						char filename[128] ;
						if( sscanf( buf+rpos , "%d %d %x %x %100s" , &gpno , &cono , &dx , &dy , filename ) != 5 )ERROR_CONT;
						...
						for( int i=0 ; i<4*8 ; i++ )
						{
							for( int q=0 ; q<64 ; q++ )
							{
								...
								{
								case 0x0000:
									chr[16*4*(i/8)+16*16*(i%8/4)+(i%4*8+q/8)*2  ] |= table[q%8] ;
								break ;
								case 0x739C:
									chr[16*4*(i/8)+16*16*(i%8/4)+(i%4*8+q/8)*2+1] |= table[q%8] ;
								break ;
								default:
									;
								}
...
#undef ERROR_CONT
		...
		if( onerror )
		{
			...
			fprintf(flog,"梱包に際し、１回以上のエラーが発生しました\n複数箇所でエラーが出ているときは、先に出ているものから解決してください\n") ;
		}
		else
		{
