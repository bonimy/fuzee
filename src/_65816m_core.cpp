...
inline static void STA(DataSet* Pd, int adr) { WRITEA(Pd, adr, Pd->reg.a.w); }
inline static void STX(DataSet* Pd, int adr) { WRITEX(Pd, adr, Pd->reg.x.w); }
inline static void STY(DataSet* Pd, int adr) { WRITEY(Pd, adr, Pd->reg.y.w); }
...
inline static bool JMP(DataSet* Pd, int adr) {
    if ((adr & 0xFFFF) < 0x8000) return false;
...
inline static void BlockCopy(DataSet* Pd, BYTE dest, BYTE src, int ID) {
    ...
    for (;;) {
        ...
        if (IS8X) {
            WRITEB(Pd, destl | Pd->reg.y.b.l, READB(Pd, srcl | Pd->reg.x.b.l));
            Pd->reg.x.b.l += ID;
            Pd->reg.y.b.l += ID;
        } else {
            WRITEB(Pd, destl | Pd->reg.y.w, READB(Pd, srcl | Pd->reg.x.w));
            Pd->reg.x.w += ID;
            Pd->reg.y.w += ID;
        }
        if (IS8A)
...
inline static int Zeropage(DataSet* Pd) { return GetROMinPCinc(Pd); }
...
inline static int StackRelative(DataSet* Pd) { return 0; }
inline static int StackRelativeIndirectY(DataSet* Pd) { return 0; }
...
#ifdef _DEBUG
    if (((d.reg.pb << 16) | d.reg.pc) == 0x0081E6EA) d.reg.a = d.reg.a;
#endif
