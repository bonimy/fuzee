...
void MyDIBObj3 :: Flip( int fliptype , int* argv )
{
	...
	break;
	case 1:
		BitBlt(hdcp,argv[0],argv[1],primarywidth,primaryheight,sfc[0].hdc,0,0,SRCCOPY);
	break;
	case 2:
		StretchBlt(hdcp,0,0,argv[0],argv[1],sfc[0].hdc,0,0,primarywidth,primaryheight,SRCCOPY);
	break;
	case 3:
		StretchBlt(hdcp,argv[0],argv[1],argv[2],argv[3],sfc[0].hdc,0,0,primarywidth,primaryheight,SRCCOPY);
	break;
	default:
		__assume(0);
	}
