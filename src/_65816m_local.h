...
#define		PCLEAR		{CLR_F(FLAG_Z|FLAG_V|FLAG_N);}
//#define		PCLEARA		{CLR_F(FLAG_Z|FLAG_V|FLAG_N);}

#define		TESTA()		{if(IS8A){TESTA8();}else {TESTA16();}}
#define		TESTX()		{if(IS8X){TESTX8();}else {TESTX16();}}
#define		TESTY()		{if(IS8Y){TESTY8();}else {TESTY16();}}
#define		TESTNUM(num)	{if(IS8A){PCLEAR;if(!num)SET_F(FLAG_Z);}else{PCLEAR;if(!num)SET_F(FLAG_Z);}}

#define		TEST8(num)	{PCLEAR; if(!num)SET_F(FLAG_Z);if(num&0x80)SET_F(FLAG_N);}
#define		TEST16(num)	{PCLEAR; if(!num)SET_F(FLAG_Z);if(num&0x8000)SET_F(FLAG_N);}

#define		TESTX8()	{TEST8 (Pd->reg.x.b.l);}
#define		TESTX16()	{TEST16(Pd->reg.x.w  );}
#define		TESTY8()	{TEST8 (Pd->reg.y.b.l);}
#define		TESTY16()	{TEST16(Pd->reg.y.w  );}
#define		TESTA8()	{TEST8 (Pd->reg.a.b.l);}
#define		TESTA16()	{TEST16(Pd->reg.a.w  );}
...
inline static int TAdr2Mem( int adr )
{
	...
	else if((adr&0xFF0000) == 0x7F0000){return (adr&0x00FFFF)|0x010000;}
	if( adr < 0x400000 )
	{
		if( (adr & 0x00FFFF) < 0x2000){return adr & 0x00FFFF;}
	}
	return -1;
}
...
int bank,loadr , pro ;
		...
		loadr = adr&0xFFFF;
		if( bank<0x40 )pro = 0 ;
		else if( bank<0x60 )pro = 1  ;
		else if( bank<0x70 )pro = -1 ;
		else if( bank<0x78 )pro = 2 ;
		else if( bank<0x7E )pro = -1 ;
		else if( bank<0x80 )pro = 3 ;
		else if( bank<0xC0 )pro = 0 ;
		else                pro = 1 ;
		switch( pro )
		{
...
{
			if( loadr>=0x8000 )
				return Pd->Prom[((bank%0x40)<<16)|loadr] ;
			if( loadr>=0x6000 )
			{
				if( bank>=0x30 )return 0 ;
				else return 0 ;
			}
			if( loadr<0x2000 )
				return Pd->Pram[loadr] ;
			//2000-5FFF
}
		break ;
		case 1:
		...
		case 3:
{
	...
	{
		if((adr&0xFF0000) == 0x7E0000)return Pd->Pram[adr&0x00FFFF];
		else if((adr&0xFF0000) == 0x7F0000)return Pd->Pram[(adr&0x00FFFF)|0x010000];
		...
		case 3:
{
	...
	{
		if((adr&0xFF0000) == 0x7E0000){Pd->Pram[adr&0x00FFFF] = data;return;}
						...
						case 0x00:
							for( q=0 ; q<end ; q++ )
							{
								WRITEB( Pd , 0x2100|Pd->AADMA[i][1] , 
									READB( Pd , q+(Pd->AADMA[i][2]|(Pd->AADMA[i][3]<<8) |(Pd->AADMA[i][4]<<16) )) ) ;
							}
						break;
						case 0x01:
							for( q=0 ; q<end ;  )
...
{
	if(IS8A){WRITEB( Pd , adr , (BYTE)data);return;}
	WRITEW( Pd , adr  , data);
	return;
}
inline static void WRITEX( DataSet *Pd , int adr , WORD data  )
{
	if(IS8X){WRITEB( Pd , adr , (BYTE)data );return;}
	WRITEW( Pd , adr  , data);
	return;
}
inline static void WRITEY( DataSet *Pd , int adr , WORD data  )
{
	if(IS8Y){WRITEB( Pd , adr , (BYTE)data );return;}
	WRITEW( Pd , adr  , data);
	return ;
}
