...
void SetSNESDrawRoutine(void (*PiCLSFUNK)(int, int, int, int, int, UINT), UINT* Pict4,
                        UINT* Pict16, UINT* Pict256) {
    PCLSFUNK = PiCLSFUNK;
    SetSNESPalette(Pict4, Pict16, Pict256);
}


void DrawSNESCharacter(BYTE* Pbuf, int sfc, int x, int y, int mode, bool emphasis,
                       int zoom) {
    BYTE tmp[8];
    int ix, iy, q;
    UINT tcolor;
        ...
        for (ix = 0; ix < 8; ix++) {
            if (mode != 2) {
                for (q = 0; q < looptable[mode]; q++) {
                    if (Pbuf[(q % 2) + q / 2 * 16 + iy * 2] & bittable[ix])
                        tmp[ix] |= bittabler[q];
                }  // q%2+q/2*16+iy*2
            } else {
